...
func (c *DBClient) Get(key []byte, ro *opt.ReadOptions) ([]byte, error) {
	resp, err := http.Get(fmt.Sprintf("%s/get/%s", c.baseURL, string(key)))
	if err != nil {
		return nil, err
	}
...
func (c *DBClient) Put(key []byte, value []byte, ro *opt.ReadOptions) error {
	payload := map[string]string{"key": string(key), "value": string(value)}
	jsonData, _ := json.Marshal(payload)
	resp, err := http.Post(fmt.Sprintf("%s/put", c.baseURL), "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
func (c *DBClient) Delete(key []byte, ro *opt.ReadOptions) error {
	req, err := http.NewRequest("DELETE", fmt.Sprintf("%s/delete/%s", c.baseURL, string(key)), nil)
	if err != nil {
		return err
	}
...
func (c *DBClient) Has(key []byte, ro *opt.ReadOptions) (bool, error) {
	resp, err := http.Get(fmt.Sprintf("%s/get/%s", c.baseURL, key))
	if err != nil {
		return false, err
	}
	defer resp.Body.Close()
...
func (c *DBClient) NewIterator(rang *util.Range, ro *opt.ReadOptions) ethdb.Iterator {
	return nil
}
...
func (c *DBClient) Stats(stats *leveldb.DBStats) error {
	return nil
}

func (c *DBClient) CompactRange(rang util.Range) error {
	return nil
}

func (c *DBClient) Write(batch *leveldb.Batch, ro *opt.ReadOptions) error {
	return nil
}
...
type RemoteLevelDbDatabase struct {
	fn string    // filename for reporting
	db *DBClient // LevelDB instance
...
func New(file string, cache int, handles int, namespace string, readonly bool) (*RemoteLevelDbDatabase, error) {
...
func NewCustom(file string, namespace string, customize func(options *opt.Options)) (*RemoteLevelDbDatabase, error) {
...
func (db *RemoteLevelDbDatabase) Close() error {
	db.quitLock.Lock()
...
func (db *RemoteLevelDbDatabase) Has(key []byte) (bool, error) {
	return db.db.Has(key, nil)
}

// Get retrieves the given key if it's present in the key-value store.
func (db *RemoteLevelDbDatabase) Get(key []byte) ([]byte, error) {
...
func (db *RemoteLevelDbDatabase) Put(key []byte, value []byte) error {
	return db.db.Put(key, value, nil)
}

// Delete removes the key from the key-value store.
func (db *RemoteLevelDbDatabase) Delete(key []byte) error {
	return db.db.Delete(key, nil)
}

// NewBatch creates a write-only key-value store that buffers changes to its host
// database until a final write is called.
func (db *RemoteLevelDbDatabase) NewBatch() ethdb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *RemoteLevelDbDatabase) NewBatchWithSize(size int) ethdb.Batch {
	return &batch{
		db: db.db,
...
func (db *RemoteLevelDbDatabase) NewIterator(prefix []byte, start []byte) ethdb.Iterator {
...
func (db *RemoteLevelDbDatabase) NewSnapshot() (ethdb.Snapshot, error) {
...
func (db *RemoteLevelDbDatabase) Stat() (string, error) {
...
func (db *RemoteLevelDbDatabase) Compact(start []byte, limit []byte) error {
	return db.db.CompactRange(util.Range{Start: start, Limit: limit})
}

// Path returns the path to the database directory.
func (db *RemoteLevelDbDatabase) Path() string {
	return db.fn
}

// meter periodically retrieves internal leveldb counters and reports them to
// the metrics subsystem.
func (db *RemoteLevelDbDatabase) meter(refresh time.Duration, namespace string) {
